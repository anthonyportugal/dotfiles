...
| 2026-09-05 | Asistente interactivo modular (`setup`), instalador `install.sh`, Multi-WM, `sync` y `update`. | Se implementó el asistente interactivo de instalación por capas (`setup`), el script de bootstrap autónomo `install.sh` compatible con curl/bash, soporte multi-WM simultáneo (MangoWM y BSPWM), comandos `sync` (re-enlazado y validación local sin tocar Git) y `update` (actualización segura remota con protección de cambios locales). Se añadieron tests automatizados en `tests/wizard-smoke.sh`. |
